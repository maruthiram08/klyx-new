...
def interpret_technical_snapshot(row):
    """
    Analyzes technical indicators present in the row (RSI, MACD, SMAs)
    Returns a structured dictionary (similar to TechnicalAnalyst).
    """
    snapshot = {
        "price": 0,
        "trend": "Unknown",
        "rsi": {"value": 0, "state": "Unknown"},
        "macd": {"signal": "Unknown"},
    ...
    sma50 = row.get('Day SMA50')
    sma200 = row.get('Day SMA200')
    ...
    if price and str(price).lower() != 'nan':
        ...
        if sma50 and str(sma50).lower() != 'nan':
            sma50 = float(sma50)
            if price > sma50:
                trend_status = "Bullish"
                signals.append(f"Price ({round(price,1)}) > 50-DMA ({round(sma50,1)}).")
            else:
                trend_status = "Bearish"
                signals.append(f"Price ({round(price,1)}) < 50-DMA ({round(sma50,1)}).")
        
        if sma200 and str(sma200).lower() != 'nan':
            sma200 = float(sma200)
            if price > sma200:
                trend_status += " (Long Term)"
                signals.append(f"Above 200-DMA.")
            else:
                # If short term is bullish but long term bearish?
                pass
                
        snapshot["trend"] = trend_status

    # 1. RSI
    rsi = row.get('Day RSI')
    if rsi and str(rsi).lower() != 'nan':
        rsi = float(rsi)
        has_data = True
        snapshot["rsi"]["value"] = round(rsi, 2)
        
        if rsi > 70:
            snapshot["rsi"]["state"] = "Overbought"
            signals.append(f"RSI Overbought ({round(rsi,1)}).")
        elif rsi < 30:
            snapshot["rsi"]["state"] = "Oversold"
            signals.append(f"RSI Oversold ({round(rsi,1)}).")
        else:
            snapshot["rsi"]["state"] = "Neutral"
            signals.append(f"RSI Neutral ({round(rsi,1)}).")

    # 3. MACD
    macd = row.get('Day MACD')
    signal = row.get('Day MACD Signal Line')
    if macd and signal and str(macd).lower() != 'nan' and str(signal).lower() != 'nan':
        has_data = True
        if float(macd) > float(signal):
             snapshot["macd"]["signal"] = "Bullish"
             signals.append("MACD Bullish Crossover.")
        else:
             snapshot["macd"]["signal"] = "Bearish"
             signals.append("MACD Bearish.")

    if not has_data:
        return None
        
    snapshot["summary_text"] = " ".join(signals)
    return snapshot
